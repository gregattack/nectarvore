...
function playNote(nt, ind, minLength){
	...
	if(noteStart !== undefined && noteDur !== undefined){
		if(noteDur >= minLength) {
			post('\nPlaying note with start:', noteStart, 'and duration:', noteDur);
			outlet(0,[noteStart, noteDur, noteStart + noteDur, nt]);
		} else {
			post('\nNote duration was too short (duration:', noteDur,'). Skipping note.');
		}
	} else {
		post('\ndoing nothing', [0, 0, 0]);
		outlet(0,[0, 0, 0]);
